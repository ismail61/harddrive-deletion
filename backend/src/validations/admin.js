...
const scanValidation = ({ orderId, serialNumber }) => {
  const schema = Joi.object().keys({
  ...
  const { value, error } = schema.validate({ orderId, serialNumber }, { escapeHtml: true });
  return { value, error };
};


module.exports = { customerValidation, scanValidation, getQueryOrdersValidation, orderValidation, getQueryCustomersValidation };